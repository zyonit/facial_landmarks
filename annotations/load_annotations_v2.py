...
#usage: load_annotations_v2.py -a <path to annotations.json file> -c <path to annotations.json file>
# it outputs to current directory:
...
parser.add_argument('-c',  type=str,
                    help='path for classes.json')
parser.add_argument('-a', type = str, help = 'path for annotations.json')
...
for imagename in annotations:
    if imagename == "___sa_version___":  # all annotationsfiles  have a '___sa_version___' field, with no important information, therefore ignore
        continue
    ...
    pupils_validation = 0
    bbox_validation =0
    annotations = annotations[imagename]["instances"]
    for label_dic in annotations:
        try:
